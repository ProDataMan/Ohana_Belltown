...
- Staff rewards (`/staff-rewards-admin.html`, admin only) — a points card (same 10-point-to-redeem mechanic as the customer loyalty card) earned by keeping the site itself up to date. Points are valued per category rather than flat, scaled to real effort/business value: marking a special or updating a price = 1 point (trivial data entry), adding a photo or a new event = 2 points (real content-creation effort), a social media post = 3 points (the most effort — shoot, write, post — and the most marketing value). Editing a single menu item (via `/edit-item.html` or the inline "Edit" link — not the bulk `/edit.html` save, since that endpoint is also used by one-off migration scripts and shouldn't rack up points for those) auto-awards whichever categories actually changed, all independently, batched into one write per edit rather than one per category. Adding a new event on `/events-admin.html` auto-awards its points too. A staff member can also log their own unverifiable activity (a social media post, or anything else) right from `/account.html` — this and every auto-award share one cap of 10 points/staff/day, so neither repeatedly toggling a field nor repeatedly self-logging can farm unlimited points. An admin can still grant any category by hand from `/staff-rewards-admin.html`, exempt from the daily cap. **The reward: 10 points = one free Classic Ohana Roll or Happy Hour appetizer** — a deliberate ~$1/point anchor, not an arbitrary count: those two lists average $8.90 and $11.00 respectively (~$9.83 combined) as of 2026-07-28, priced from the already-discounted Happy Hour menu so the cost per redemption stays predictable. An admin redeems it once a staff member gets there, with an optional note for what was actually given. Pre-2026-07-28 data (flat 1-point-per-action) decodes forward compatibly — no migration needed.
