...
| `STRIPE_SECRET_KEY` | Server-side only — powers real card checkout on the Shop page (Stripe Checkout Sessions). Not set yet. Get one at [dashboard.stripe.com](https://dashboard.stripe.com) (API Keys page) — needs a Stripe account with payouts set up to actually receive money, though the key itself is free to generate. Until set, the Shop page still shows products/prices but the checkout button stays hidden. |
| `STRIPE_WEBHOOK_SECRET` | Server-side only — verifies that `POST /api/swag/stripe-webhook` calls genuinely came from Stripe before marking a swag order paid. Created when you add a webhook endpoint in the Stripe Dashboard pointed at `<PUBLIC_BASE_URL>/api/swag/stripe-webhook`, subscribed to the `checkout.session.completed` event. Not set yet. |
...
- Competitor Menu Pricing (`/competitor-pricing-admin.html`, admin only, plus a summary on `/analytics.html`) — compares our prices for a dish (e.g. "Ohana Burger," "Spicy Tuna Roll") against nearby restaurants' published prices for the same/similar item, so pricing can be checked against the neighborhood instead of guessing. The restaurant list is sourced from Google Maps' own Nearby Search (`GET /api/competitor-pricing/nearby-restaurants`, real distance computed from Ohana's own coordinates via the Google Place Details `geometry` field, 3 miles by default) — staff pick from real, current nearby restaurants instead of typing a name/address by hand. Google has no equivalent API for a competitor's actual menu *prices*, though (and most menu/delivery-app sites block automated fetching), so those still need a person to check the competitor's own site/menu and log what they find, with an optional source link and the date checked — a competitor restaurant can also have its own menu photos uploaded (same shared `/api/upload` used for menu item photos) as a reference for whoever's entering prices, since the competitor's own site often doesn't list them at all. The admin page opens with a numbered "How to Add a Comparison" walkthrough (find a restaurant → create a comparison group → look up their price → log it as an entry → check the report) so the workflow doesn't have to be re-explained. The report shows our price (pulled live so it can't go stale) alongside the competitor average/min/max and how far off we are (in dollars and %), per comparison.
...
- **Shop checkout is built but not live yet.** Needs `STRIPE_SECRET_KEY` + `STRIPE_WEBHOOK_SECRET` (see env table above) — a Stripe account isn't set up yet. Until then, `/shop` shows the 3 swag products with real prices, but the checkout button stays hidden (checked via `GET /api/swag/checkout-status`), same hidden-until-configured pattern. Products themselves (name/price/photos) can be managed on `/swag-admin.html` regardless.
