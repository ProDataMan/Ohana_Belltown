...
- Competitor Menu Pricing (`/competitor-pricing-admin.html`, admin only, plus a summary on `/analytics.html`) — compares our prices for a dish (e.g. "Ohana Burger," "Spicy Tuna Roll") against nearby restaurants' published prices for the same/similar item, so pricing can be checked against the neighborhood instead of guessing. There's no reliable live API for a competitor's actual menu prices, so this is entirely staff-curated: add a competitor restaurant, a comparison dish (optionally linked to one of our own menu items, so our side of the comparison always reflects the live current price rather than a number that can go stale), then log competitor prices as they're found (their own menu/site/delivery app) with an optional source link and the date checked. The report shows our price alongside the competitor average/min/max and how far off we are (in dollars and %), per comparison.
