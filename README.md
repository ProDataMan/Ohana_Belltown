...
- `/entertainment` (2026-09-08) — a static weekly nightlife schedule page (`server/Public/pages/entertainment.html`, registered as a clean route in `routes.swift`'s `cleanPages` list): no entertainment Monday/Tuesday, Island Nights (free live island music) Wednesday at 9pm, DJ nights Thursday–Saturday, karaoke Sunday. Deliberately static/low-maintenance rather than admin-editable — the specific DJ/performer each week already gets posted to Ohana's Facebook/Instagram, so the page links out to both (prominent CTA buttons, not just the footer icons) for the current week's lineup and event photos rather than trying to duplicate that content here. Linked from the main nav (all 21 public/customer pages) and a new homepage callout section, same pattern as Rewards/Gift Cards/Catering.
- **Island Nights promoted on the homepage** (2026-09-08) — the homepage's generic "Entertainment" panel (plain text, no image — one of the sections still flagged after the 2026-09-07 imagery pass) is now a dedicated "Island Nights" `content-grid` section, matching the treatment already given to Sushi Punch Card/Catering, with a real photo of a live band performing at Ohana (`/images/island-nights.jpg`, pulled from the archived pre-redesign site's own photo library — genuine venue content, not stock). `/entertainment.html`'s own Island Nights section was switched to the same photo (previously reused `hero-contact.jpg`, the dining room shot also used for homepage Catering — now freed back down to two uses instead of three) and both pages' copy/heading were corrected from "Island Night" to the actual name, "Island Nights."
- **Island Nights performer roster + a new restricted staff role** (2026-09-08) — `/island-nights-admin.html` lets staff book a named performer per upcoming Wednesday with photos and a video (with sound), backed by a new `IslandNightsStore` (`Data/island-nights.json`). `/entertainment` now has a "Coming Up on Island Nights" section (`island-nights-performers.js`) pulling from `GET /api/island-nights/upcoming`, reusing the gallery lightbox for photos and a native `<video>` element for the clip. Videos go through a new `POST /api/upload-video` (200mb ceiling, mp4/mov/webm/m4v) separate from the existing image-only `/api/upload` (kept at 20mb) — served back by the same `/uploads/:filename` route, which already supports Range requests for free, so `<video>` seeking works with no extra code.

  This shipped alongside a new **`entertainmentProvider`** role — the first real access restriction beyond admin-vs-employee. It's for outside DJs/promoters who need to manage their own Island Nights listing but shouldn't reach the menu, table orders, loyalty data, or anything else. A new `requireStaffAccess(_:)` (Auth.swift) is `requireLogin` plus an explicit exclusion of this role, and now gates every staff route that isn't Island Nights, the video/photo upload, or a staff member's own account fields (~35 route call sites audited and switched over — see git history for the full list). `serveStaffPage` enforces the same boundary for whole pages via a small allowlist (`island-nights-admin.html`, `account.html`, `change-password.html`, `help.html`); everything else 403s regardless of the page's own `adminOnly` flag. `staff-auth.js` (loaded on every staff page) additionally trims the shared `.staff-tools-nav` down to that allowlist client-side when it detects this role, so the nav doesn't show a page full of links that would 403 — presentation only, the server-side checks are what actually matter. `create-account.html` and `manage-users.html`'s role dropdown both support assigning it.

  A performer entry also carries an optional **start time** (`startTime`, "HH:mm", defaults to 9pm client-side) alongside its date — set from a native `<input type="time">` on `/island-nights-admin.html`, shown on `/entertainment`'s "Coming Up" cards as e.g. "Wednesday, September 30 · 9:00 PM". `/island-nights-guide.html` is a printable (not staff-gated, same pattern as `/table-card.html`) one-page walkthrough — log in, open Island Nights, add date/time/name/bio/photos/video — meant to be printed and handed to a new DJ or promoter along with their username/temporary password; linked from a new Island Nights section on `/help.html`.

  entertainmentProvider accounts are further limited to a **rolling 60-day booking window** (`IslandNightsStore.isWithinEntertainmentProviderWindow`, checked in the create/update/delete routes) — they can't create, edit, or delete a performer entry dated more than 60 days out or in the past, and can't move an in-window entry outside that window either. Admin/employee accounts have no such limit. The window check computes against `America/Los_Angeles` via an explicit `Calendar` (not `Calendar.current`) to avoid DST-boundary miscounts.
