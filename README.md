...
- `/entertainment` (2026-09-08, restructured 2026-09-08) — a weekly-lineup summary page (`server/Public/pages/entertainment.html`, registered as a clean route in `routes.swift`'s `cleanPages` list): no entertainment Monday/Tuesday, Island Nights (free live island music) Wednesday at 9pm, DJ nights Thursday–Saturday, karaoke Sunday. Each row in the schedule table links out to that night's own page (see below) for the actual lineup/photos/video; the page also links out to Facebook/Instagram (prominent CTA buttons, not just the footer icons) for anything not yet posted here. Linked from the main nav (all 21 public/customer pages) and a new homepage callout section, same pattern as Rewards/Gift Cards/Catering.
- **Island Nights promoted on the homepage** (2026-09-08) — the homepage's generic "Entertainment" panel (plain text, no image — one of the sections still flagged after the 2026-09-07 imagery pass) is now a dedicated "Island Nights" `content-grid` section, matching the treatment already given to Sushi Punch Card/Catering, with a real photo of a live band performing at Ohana (`/images/island-nights.jpg`, pulled from the archived pre-redesign site's own photo library — genuine venue content, not stock) and a "See Who's Playing" link straight to `/entertainment/wednesday`.
- **A page per night, all admin-editable, plus a restricted staff role** (2026-09-08) — what started as a Wednesday-only "Island Nights" feature was generalized the same day into one system covering all 7 nights. `/entertainment-admin.html` lets staff book a named performer for any upcoming date — name, start time, bio, photos, and a video (with sound) — backed by `EntertainmentStore` (`Sources/App/Entertainment.swift`, `Data/entertainment.json`). Which of the 7 public pages a booking shows up on is *derived from its date* (`EntertainmentStore.weekday(of:)`), never stored separately, so it can't drift out of sync with the date. Each night has its own page at `/entertainment/<day>` (`monday` through `sunday`) — one shared template (`pages/entertainment-night.html` + `entertainment-night.js`) reads which day from the URL itself (`location.pathname`, same pattern as `window.MENU_SECTION` on the shared menu pages) rather than needing 7 near-identical HTML files; an unknown day 404s (`routes.swift`'s `validWeekdays` check). `GET /api/entertainment/upcoming` optionally takes `?weekday=wednesday` to filter server-side. Videos go through `POST /api/upload-video` (200mb ceiling, mp4/mov/webm/m4v) separate from the existing image-only `/api/upload` (kept at 20mb) — served back by the same `/uploads/:filename` route, which already supports Range requests for free, so `<video>` seeking works with no extra code.

  This shipped alongside a new **`entertainmentProvider`** role — the first real access restriction beyond admin-vs-employee. It's for outside DJs/promoters/karaoke hosts who need to manage their own booking(s) but shouldn't reach the menu, table orders, loyalty data, or anything else. A new `requireStaffAccess(_:)` (Auth.swift) is `requireLogin` plus an explicit exclusion of this role, and gates every staff route that isn't entertainment bookings, the video/photo upload, or a staff member's own account fields (~35 route call sites audited and switched over — see git history for the full list). `serveStaffPage` enforces the same boundary for whole pages via a small allowlist (`entertainment-admin.html`, `account.html`, `change-password.html`, `help.html`); everything else 403s regardless of the page's own `adminOnly` flag. `staff-auth.js` (loaded on every staff page) additionally trims the shared `.staff-tools-nav` down to that allowlist client-side when it detects this role, so the nav doesn't show a page full of links that would 403 — presentation only, the server-side checks are what actually matter. `create-account.html` and `manage-users.html`'s role dropdown both support assigning it.

  A booking also carries an optional **start time** (`startTime`, "HH:mm", defaults to 9pm client-side) alongside its date — set from a native `<input type="time">` on `/entertainment-admin.html`, which also shows a live label (e.g. "Wednesday, September 30 — Island Nights") as you pick a date, computed the same way the server derives which night a booking belongs to. `/entertainment-guide.html` is a printable (not staff-gated, same pattern as `/table-card.html`) one-page walkthrough — log in, open Entertainment, add date/time/name/bio/photos/video — meant to be printed and handed to a new DJ, performer, or karaoke host along with their username/temporary password and which night they're playing (a blank line on the printout); linked from an Entertainment section on `/help.html`.

  entertainmentProvider accounts are further limited to a **rolling 60-day booking window** (`EntertainmentStore.isWithinEntertainmentProviderWindow`, checked in the create/update/delete routes) — they can't create, edit, or delete a booking dated more than 60 days out or in the past, and can't move an in-window one outside that window either. Admin/employee accounts have no such limit. The window check computes against `America/Los_Angeles` via an explicit `Calendar` (not `Calendar.current`) to avoid DST-boundary miscounts.
- **Favicon and app icons** (2026-09-08) — the site never had a favicon at all before this. `favicon.ico` at the Public root covers the default browser behavior (most browsers request that path automatically with zero markup); `nav.js` (loaded on every page) additionally injects `<link rel="icon">` (crisper PNG) and `<link rel="apple-touch-icon">` tags into `document.head` at runtime, rather than duplicating `<link>` tags across 20+ HTML files individually. Both derived from the existing `logo.png` wordmark: the small favicon sizes (16/32px) use a solid-purple background since the script wordmark turns to mush at that size over a gradient; `images/app-icon-1024.png` (and a 512px version) use the full brand gradient, legible at the larger sizes Facebook/Google's app-icon upload fields actually display at. No square/monogram brand asset existed anywhere in the repo to start from — these are generated from the wordmark, not a pre-existing icon.
...
- **Fixed** (2026-09-15): every item row's only signal that it opened a detail view was `cursor: pointer` — invisible on the touchscreens that make up most of this site's traffic, and the row also had no keyboard path at all (no `tabindex`, no `Enter`/`Space` handler). Each non-sold-out row now shows a small "Tap for details ›" cue and is a real `role="button" tabindex="0"` with a matching keydown handler and an accessible name. See the [Menu Engagement Playbook](https://claude.ai/code/artifact/b5d70e9f-a2fe-4bd1-b3ad-d6b9e9c1de3f) for the research behind it and further recommendations (photo/description coverage gaps, etc.).
- Photo taps and details taps on a menu item are now tracked separately, per item and per visitor — `POST /api/analytics/menu-item-engagement` (fired from both the card's photo and the rest of the row, `menu-section.js`), attributed to the same per-browser device id order history already uses (see below), plus the signed-in customer id when there is one. Deliberately a separate store (`MenuItemEngagementStore`, `Data/menu-item-engagement.json`, 120-day retention) from the older, anonymous `item-view` counter behind "most-viewed menu items" below — that one stays identity-free exactly as documented. `GET /api/analytics/menu-item-engagement` (admin-only) returns click totals and unique-visitor counts per item per click type; no dashboard panel built on it yet.
- Tap rate, not just a raw tap count: an `IntersectionObserver` (`observeItemImpressions()`, `menu-section.js`) now records when a card is actually scrolled into view (50%+ visible, counted once per item per page visit, queued and flushed in batches — `POST /api/analytics/menu-item-impressions` — rather than one request per card on a 200+-item menu; flushed via `navigator.sendBeacon` on tab-hide so a quick visit isn't lost). `GET /api/analytics/menu-item-tap-rate` (admin-only) divides taps by impressions per item, so a card near the top of a long category (seen by everyone scrolling past) is no longer indistinguishable from one that's genuinely earning taps — an item never shown reports a `null` rate rather than a misleading 0.
- Closed the description gap the [Menu Engagement Playbook](https://claude.ai/code/artifact/b5d70e9f-a2fe-4bd1-b3ad-d6b9e9c1de3f) flagged: all 12 items in the reference menu (`Resources/seed-menu.json`) that had no description — 8 Extra & Sides items, 4 Drinks categories — now have one. This is the bootstrap/dev fixture, not the live production menu (that's separately staff-edited data on the Azure Files volume, outside this repo) — the same fix still needs doing there via `/edit.html`. The photo-coverage gap the same report flagged (210 of 216 reference-menu items have no photo) is a photography task, not something fixable from this codebase.
...
- **Automatic punches for signed-in customers** (2026-09-08) — until now every punch was 100% manual (staff had to remember to ask for a phone number and key it in on `/loyalty-admin.html`), with zero link to the real table-order data the system already had. Now, when a signed-in customer's table order is marked delivered and it's a real `/sushi`-page item (`TableOrderEntry.section == "sushi"` — the menu section, not the table's physical floor-map section, so a sushi roll ordered from Happy Hour still needs a manual punch, unchanged from before) and that customer has linked a rewards phone (`/my-account.html`), their card gets punched automatically (`POST /api/table-orders/:id/deliver` in `routes.swift`, best-effort — never blocks delivery itself). Capped at one automatic punch per calendar day per phone (`LoyaltyCustomer.lastAutoPunchDay`), independent of manual staff punches, so ordering several rolls in one sitting doesn't fill a card in a single visit. Anonymous/guest orders are unaffected — staff still ask for a phone and punch by hand, exactly as before.
- **Automatic birthday bonus punch** (2026-09-08) — a daily sweep (`LoyaltyBirthdayBonus.swift`, registered in `configure.swift` as a 24-hour repeating task) checks `CustomerUserStore.upcomingBirthdays(withinDays: 0)` (the same function that already powers staff's "birthdays in the next 7 days" list) for any signed-in customer whose birthday is today and who has a linked rewards phone, and credits their card a bonus punch. Idempotent per calendar year (`LoyaltyCustomer.lastBirthdayBonusYear`), so the sweep can safely run every day without double-paying if it happens to fire more than once.
- **Referral bonus** (2026-09-08) — a new "Refer a Friend" panel on `/rewards` lets a brand-new phone number record who referred them (`POST /api/loyalty/referral`, rejects self-referral and rejects a phone that already has a card, so an existing customer can't backfill a referrer to farm a bonus). The referrer's bonus punch pays out at the referred customer's *first real punch* — not at signup — so it can't be farmed without an actual visit; fires exactly once (`LoyaltyCustomer.referralBonusPaid`) whether that first punch comes from staff manually punching a card or the new automatic sushi-order punch above.
- **Promotion** (2026-09-08) — the punch card was previously only mentioned in the site nav bar, with no homepage callout (unlike Gift Cards/Catering, which each get one), nothing on the printed table QR cards, and nothing in the ordering flow itself. Fixed on all three fronts: a new "Sushi Punch Card" section on the homepage (same pattern as the existing Gift Cards section) linking to `/rewards`; a third standalone printable QR card on `/table-card.html` ("Join the Sushi Punch Card," alongside the existing "While You Wait"/"Shop Ohana Swag" cards); and a one-line reward mention appended to the ordering hint banner specifically on `/sushi` (`window.MENU_SECTION === 'sushi'` in `menu-section.js`) — the one page where a guest is actively about to order sushi. The homepage Sushi Punch Card and Catering sections were later (2026-09-07) given real photos (a sushi-platter shot and a shot of the dining room set up for an event, respectively), matching the `content-grid` image+text pattern already used elsewhere on the page instead of the plain centered-text card the rest of the homepage's info sections still use.
- **Login-gated punch card on `/rewards`** (2026-09-07) — the public "Check My Card" phone-number box (anyone could look up anyone's punch balance by typing any phone number) is now a login-gated flow reusing account infrastructure that already existed on `/my-account.html`: signed out, `/rewards` shows a Log In / Create Account prompt (`?next=/rewards`, so both land back on `/rewards` after — `signup.js` didn't previously respect `?next` and now does, matching `account-login.js`); signed in with no phone linked yet, a "add your phone number" form (`POST /api/customer/loyalty-phone`); signed in with a phone linked, the live punch count (`GET /api/customer/loyalty`) with a "Change phone number" control. The linked phone also autofills the "Your phone number" field on the still-anonymous Refer a Friend / Submit a Bonus Punch forms below it, so a signed-in guest doesn't retype it. `POST /api/loyalty/lookup` itself is intentionally untouched — `/loyalty-admin.html` still uses it for staff to look up any card by phone at the table, unrelated to this change.
- **A real visual punch card, not just a number** (2026-09-14) — the signed-in "My Punch Card" state on `/rewards` (see above) previously rendered as a plain "N / 10 punches" pill. It's now `images/punch-card-bg.png` (a generated card — purple brand gradient, gold frame, the Ohana wordmark, "10 PUNCHES = ONE FREE ROLL" — with its lower half deliberately left visually clear) plus a live 10-dot overlay (`.punch-card-dots` in `style.css`) — however many punches the guest actually has render as filled gold circles with a checkmark, the rest stay outlined. The dot count is capped at 10 even if the server total is briefly higher (a reward earned but not yet redeemed at the register), so the grid never overflows; "Free roll ready" still shows as a pill underneath once all 10 are filled. Punch count and card art are two separate layers on purpose — the count is different for every guest and changes in real time, so it's never baked into the image itself. The rendering logic (`renderPunchCardInto`) lives in a new shared `punch-card.js` rather than inside `rewards.js`, specifically so `/my-account.html`'s own card display (below) can't drift out of sync with it again.
- **Loyalty program refinements** (2026-09-15) — a batch of fixes from reviewing the program end to end:
  - `/my-account.html`'s Rewards Card now renders the same real punch-card image as `/rewards` (`punch-card.js`) instead of the old plain-pill design it had kept using after `/rewards` moved on.
  - **Fixed a real trap in "Refer a Friend."** `LoyaltyStore.setReferrer` only accepts a phone that's never punched before, but the form was auto-filling "Your phone number" with the logged-in guest's own linked phone — meaning any existing customer who tried to refer a friend using their own account got a guaranteed, confusing rejection. A customer who already has a card now sees a shareable link (`/rewards?ref=<phone>`) instead of the self-submit form; a friend who opens that link gets their "Referred by" field auto-filled, so they never have to know or type a phone number by hand. The form is unaffected for a guest who doesn't have a card yet.
  - The birthday bonus (shipped 2026-09-08, see above) was real but invisible — nothing on `/rewards` or `/my-account.html` told a customer it existed. Both now say so explicitly: a bullet in `/rewards`' "How It Works," and updated copy on the Birthday Club field itself.
  - `/loyalty-admin.html` gets a new "Program at a Glance" stat grid (total cards, rewards ready now, lifetime free rolls redeemed, active in last 30 days, referral bonuses paid, birthday bonuses paid this year, shares approved/pending) — computed client-side from the same `/api/loyalty/customers` and `/api/loyalty/bonus-requests` responses the page already loads, no new backend endpoint. Deliberately doesn't attempt a "punches given out this week" figure — `LoyaltyCustomer` only stores a running total, not a per-punch event log, so that number genuinely can't be answered from what's stored today.
  - `docs/loyalty-points-migration.md` was stale — it described referral and birthday bonuses as hypothetical/unbuilt when both have been live since 2026-09-08. Corrected, and added a note that the new card-image display (10 fixed dot slots) is itself a consideration for any future points redesign, since an arbitrary points balance doesn't map onto "10 filled dots" the way a punch count does.
- **Redemption cap** (2026-09-15) — a real economic ceiling on what a free-roll redemption can cost, prompted by pushing the program harder while planning a swag giveaway (real swag-cost/food-cost numbers deliberately deferred until there's launch data; only the enforcement mechanism shipped now). `POST /api/loyalty/redeem` now takes a `menuItemId` alongside the phone number — `LoyaltyStore.redeem` looks the item up via `MenuStore.findItem(id:)` and rejects the redemption (400) if the item has no price or its price exceeds the stored cap (`LoyaltyData.maxRedemptionPrice`, defaulting to **$12.00**, roughly the average Rolls-category menu price so a typical redemption is a normal roll). The cap is readable by anyone (`GET /api/loyalty/redemption-cap` — `/rewards`' "How It Works" copy states the real number instead of a hardcoded one) but only an admin can change it (`PUT /api/loyalty/redemption-cap`, `requireAdmin`, plain employees get a 403) — deliberately not the same bar as reading/writing punches, since this number controls program economics rather than day-to-day card operations. `/loyalty-admin.html`'s redeem control gained a menu-item `<datalist>` picker (same pattern as the bonus-punch dish field) so staff pick a real item rather than typing a price, plus an admin-only panel to edit the cap itself.
- **Receipt-based punches** (2026-09-15) — the automatic sushi-order punch (above) only fires for orders placed through this site's own table-order queue, which is now paused (see "Table ordering phased rollout" above), and staff-manual punching was always the only fallback for anything ordered through ChowNow or rung up at the register. `/rewards`' existing staff-reviewed bonus-punch form gained a third claim type, "A receipt from today," alongside the existing photo/social options — reuses the same photo-upload field and staff review queue (`BonusRequest.type == "receipt"`, no menu item required since a receipt isn't about one dish), but an approved receipt is worth a **full punch** rather than 1/10 of one, capped at one receipt-punch per calendar day per phone (checked against the receipt's submission day, not the review day, so staff reviewing several days' backlog at once can't be gamed into awarding more than one for the same day). Staff are told to check the visible date on the receipt before approving — there's no automated date/OCR verification, the same trust model as approving a dish photo.
...
- **Table ordering phased rollout** (2026-09-15) — deploying the full item-by-item order system (below) straight to every table felt like too big a jump for staff to absorb at once, so it now launches in 3 admin-selectable phases instead of all at once: **1. Menu with prices only** (current default) — scanning a table's QR code still carries the table id along exactly as before, and prices still show, but no ordering UI renders anywhere; **2. + "Ready to Order" alert** (not built yet, reserved in the phase enum for when it's wanted) — a single button alerting staff a table wants to order, with no item-level detail; **3. Full ordering** — the complete system described in the next bullet, unchanged. Nothing about the ordering feature was removed to make this possible — `OrderSystemStore` (`Sources/App/OrderSystemSettings.swift`, `Data/order-system.json`) holds one setting (`GET /api/order-system/phase`, public; `PUT`, `requireAdmin` — same bar as the loyalty redemption cap, since flipping it is a whole-site decision) that `menu-section.js` reads before its first render and gates every "Add to Order" surface on (`canOrder = orderSystemPhase === 'fullOrdering' && (tableId || customerLoggedIn)`), defaulting to the paused phase if the fetch itself ever fails. An admin-only panel on `/table-orders-admin.html` (hidden from other staff, mirroring the redemption-cap panel on `/loyalty-admin.html`) switches between phases with no redeploy needed.
...
- Table-less pickup ordering — a signed-in customer sees "Add to Order" on every menu page even without ever scanning a table's QR code (`customerLoggedIn`, resolved via `GET /api/customer/me` before the menu first renders, OR'd everywhere `tableId` previously gated ordering). Sending the order posts an empty `tableId`, which the server accepts only when a customer session exists (an anonymous guest still gets the existing 400 if `tableId` is blank) and stores as the sentinel table id `"pickup"` — chosen over making `tableId` optional so it flows through the existing per-table grouping/floor-map code with zero special-casing, except one exclusion: `tableOccupancyStats()` skips `"pickup"` entries since a counter pickup isn't a real dine-in seating and would otherwise skew table-turnover estimates. `/table-orders-admin.html` shows a "Pickup: `<name>`" pill with the customer's photo instead of a table number for these orders, since there's no table to point staff to (see nickname/photo below for how `<name>` is chosen).
- Nickname (`/my-account.html`, `POST /api/customer/nickname`) — what a customer wants to be called at the restaurant, separate from their account display name (e.g. a Google account named "Ronald Smith" who everyone actually calls "Ronnie"). `CustomerUser.preferredName` (nickname if set, else display name) is snapshotted onto a pickup order's `customerName` at the moment it's placed — a snapshot, not a live lookup, so a later nickname change doesn't retroactively rewrite past order history. `customerPhotoURL` is snapshotted the same way, straight from the Google-sign-in-captured profile photo already shown on `/my-account.html`.
...
- **Compact review summary on `/menu`** (2026-09-08) — a small star-rating + review-count line (`menu-review-summary.js`) now sits above the Food/Sushi/Drinks/Specials/Happy Hour tabs, pulling `overallRating`/`totalRatings` off the same `GET /api/place-reviews` the homepage's full review-card carousel already uses (`reviews-widget.js`) — deliberately a compact one-liner rather than the homepage's full cards, since a guest here is about to order, not browse testimonials. Stays hidden if Google Places isn't configured or returns no rating, same "best-effort" pattern as the rest of the Places integration. Scoped to `/menu` only, matching where the original design review asked for it — not duplicated onto `/sushi`/`/drinks`/etc.
- **Fixed a broken hero image on `/entertainment` and `/happy-hour`** (2026-09-08) — `hero-happy-hour.png` was a corrupt 557-byte solid-black placeholder (not a real photo), silently unnoticeable most places since the hero banner's own dark gradient scrim (`.hero-banner::before`) already darkens whatever's underneath — but still a data-integrity problem, not a design choice. Both pages now use the real `hero-aloha.jpg` (the neon sign at night) instead — chosen for being the one genuinely nighttime/moody photo in the asset library, matching an entertainment/happy-hour theme better than the daytime alternatives. Neither page had a second use of that same image lower on the page, except `/entertainment`'s own Island Night section, which was swapped to `hero-contact.jpg` (the dining room shot, also used on the homepage's Catering section) to avoid showing the same photo twice on one page.
- **"New here?" eating-order guide on `/menu`** (2026-09-08) — a short line above the section tabs suggests an order (pupus to share, then poke/musubi, then entrée or rolls, dessert last) — a pattern several Nobu locations use ("Secrets from the Kitchen") found during a competitive scan of 15 sushi/tiki restaurant sites. Static copy, no backend involved.
- Embedded map on the Contact page (no API key/billing needed — uses the classic `maps.google.com/maps?...&output=embed` URL)
- `/gallery` — aggregates all distinct menu item photos and the rotating Google Places photos into one browsable page, filterable by All/Food/Drinks/Atmosphere tabs (2026-09-07) — a menu category's own `section` field (menu/sushi/happy_hour vs. drinks, plus a name check since happy_hour bundles a "Drinks" category alongside food ones) buckets menu-item photos into Food or Drinks; Google Places photos aren't tied to any one dish, so they land under Atmosphere rather than being guessed at. No "Events" tab — nothing in the codebase produces an event-tagged photo yet, so that bucket would just always be empty.
...
- Named username/password logins (bcrypt-hashed) with three roles: `admin`, `employee`, and `entertainmentProvider` (2026-09-08 — a deliberately narrow role for outside DJs/promoters, restricted to Island Nights only; see "Island Nights performer roster" above)
...
- Self-service account deactivation (`/my-account.html`) — immediately ends the session and blocks future login (password or OAuth), reversible by contacting the restaurant
- Self-service account deletion (`/my-account.html`, separate button/confirmation from deactivation) — permanently erases the account (name, email, photo, any OAuth link) rather than just flagging it inactive; not reversible. Also reachable indirectly: a Facebook data-deletion request (via Facebook's own settings) triggers the same erasure automatically — see the Facebook Data Deletion callback under OAuth sign-in below.
...
- Nickname (`/my-account.html`) — self-service, optional, cleared with a blank value — lets a customer tell staff what they're actually called at Ohana (may differ from their account/Google display name). Used to identify table-less pickup orders on `/table-orders-admin.html` (see "Table ordering" above) since those have no table number to show instead.
...
- Apple's button is still hidden site-wide for now (`hidden` attribute + a CSS rule, easy to re-enable) until its credentials are set up — see [`docs/oauth-setup.md`](docs/oauth-setup.md)
- **Google and Facebook are both configured** (Google since 2026-07-25, Facebook since 2026-09-08 — see [`docs/oauth-setup.md`](docs/oauth-setup.md)). Both buttons are live across `/signup`, `/account-login`, `/login`, and the account-linking button on `/account.html`. Apple is fully built underneath but stays hidden until its credentials exist. X/Twitter and Instagram were deliberately not built — X's API pricing for third-party login is unclear/likely paid, and Instagram isn't designed as a general-purpose login provider.
- **Facebook app is in Development Mode** until switched to Live in the Facebook developer dashboard (Basic Settings → requires a privacy policy URL, `https://www.ohanasushigrill.com/privacy` already works) — until then, only accounts added as testers/admins under App Roles can actually complete Facebook sign-in; everyone else gets an error from Facebook itself, not from this app.
- **Facebook Data Deletion callback** (`POST /auth/facebook/data-deletion`, `FacebookDataDeletion.swift`) — required by Facebook App Review before Login can go Live. When a user asks Facebook (via Facebook's own settings, not ours) to delete their data, Facebook POSTs a signed request here; the app verifies it (HMAC-SHA256 against the App Secret, `FacebookDataDeletion.parseUserId`) and deletes any matching customer account outright or, for a staff account, just unlinks the Facebook connection (a staff record is a real employment record the restaurant created, not something Facebook created, so the whole account isn't in scope). Responds with a confirmation URL pointing at `/data-deletion-status`, a static confirmation page. Configured in Facebook's dashboard as the **Data Deletion Request URL** — see [`docs/oauth-setup.md`](docs/oauth-setup.md).
...
- Automated test suite (`server/Tests/AppTests`, 307 tests) — loyalty punch/redeem math (including automatic sushi-order punches, birthday bonuses, referral bonuses, the redemption cap, and receipt-claim punches), the order-system phase's public-read/admin-write boundary, waitlist and table-order lifecycle behavior (including prep-time estimation, delivery-stats aggregation, cancellation, and table-less pickup orders), analytics aggregation (including OS/browser/device-model user-agent parsing), single-item menu CRUD, staff and customer auth (including deactivation, self-service account deletion, OAuth linking/photo- and email-backfill for both account types, Facebook as an independent provider alongside Google/Apple, the Facebook Data Deletion callback's signature verification and its customer-delete/staff-unlink behavior, nicknames, and rewards-card linking), entertainment booking CRUD/date-math including the date-to-weekday derivation (`EntertainmentStoreTests`) and the `entertainmentProvider` role's route- and page-level access boundaries, the weekday-filtered upcoming query, and the 7 per-night pages' routing (`RouteTests`), menu-item engagement tracking (photo vs. details clicks rolled up per item with correct unique-visitor de-duplication, plus tap-rate combining impressions and taps with a correct nil-vs-zero distinction for never-shown items), menu backward-compat decoding, the Happy Hour time-window/QR-redirect logic (including table-id passthrough), competitor-photo review tracking, station-light cue planning, and route-level permission boundaries. Run with `swift test` from `server/` — see [Local development](#local-development) for the GCC-version workaround this sandbox needs to actually build/link locally rather than relying only on CI.
...
- Self-hosted analytics (`/analytics.html`, admin only) — pageview counts by page and by day, device type (mobile/tablet/desktop), operating system (iPhone/iPad/Android/macOS/Windows/Linux), browser (Safari/Chrome/Firefox/Edge/Samsung Internet/Opera), best-effort Android hardware model (e.g. "Pixel 8" — parsed only when a browser's user-agent happens to include it; iOS never exposes a specific model, so there's no equivalent list for Apple devices), average time on page, and most-viewed menu items (detail-popup opens — a proxy for interest, not a sales figure, since this site has no access to real order data from ChowNow). No cookies, no third-party tracking script, no per-visitor identity anywhere. Bounded to 120 days of aggregated data. (The device/customer-attributed photo-vs-details tap tracking under Menu experience above is a deliberately separate, non-anonymous store — this promise is about this pageview/device/browser data specifically, not a site-wide guarantee.)
...
- Staff accounts can be deactivated/reactivated (see above), but not deleted outright — a Facebook-linked staff account only has that link removed on a Facebook data-deletion request, never the account itself. Customers can either self-deactivate (`/my-account.html`; reactivation isn't self-service, contact the restaurant) or permanently self-delete their account outright (same page, separate confirmation) — and there's still no staff-facing UI to manage/browse customer accounts.
