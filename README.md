...
- Staff rewards (`/staff-rewards-admin.html`, admin only) — a punch card (same 10-punch mechanic as the customer loyalty card) earned by keeping the site itself up to date. Editing a single menu item (via `/edit-item.html` or the inline "Edit" link — not the bulk `/edit.html` save, since that endpoint is also used by one-off migration scripts and shouldn't rack up punches for those) auto-awards a punch for adding a photo, setting a price, and/or marking it a special, all independently (batched into one write per edit, not one per category). Adding a new event on `/events-admin.html` auto-awards one too. A staff member can also log their own unverifiable activity (a social media post, or anything else) right from `/account.html` — this and every auto-award share one cap of 5 punches/staff/day, so neither repeatedly toggling a field nor repeatedly self-logging can farm unlimited punches. An admin can still grant any category by hand from `/staff-rewards-admin.html`, exempt from the daily cap. **The reward: 10 punches = one free Classic Ohana Roll or Happy Hour appetizer** (deliberately mirroring the already-discounted Happy Hour menu so the cost per redemption stays predictable, roughly $6.50&ndash;$12) — an admin redeems it once a staff member gets there, with an optional note for what was actually given.
