...
| `/table-card.html` | Printable QR-code table tents — one per numbered table (12 by default), each QR unique to its table |
...
| `/table-orders-admin.html` | Staff: "Needs Entry" and "Awaiting Delivery" queues for dine-in table orders, plus the staff-on-duty count used for prep-time estimates. **Any logged-in employee.** |
...
- Printable QR-code table tents (`/table-card.html`) — one per numbered table (12 by default, easy to add more), each QR unique to its table and encoding `ohanasushigrill.com/scan?table=N`. `/scan` checks the current day/time (Pacific) and sends the guest straight to `/happy-hour` during Happy Hour (Mon&ndash;Fri, 3&ndash;6pm) or `/menu` otherwise, carrying the table id along. Happy Hour's page carries a clear "See the Full Menu" link back out, since a scan-in visitor may not want to hunt for the section tabs. Uses `ohanasushigrill.com` (the restaurant's real domain) as the printed URL now, ahead of DNS being pointed at this site — those links go live automatically once DNS switches over.
- Table-side ordering, with a real lifecycle — once a guest has scanned a numbered table's QR code, every menu item on `/menu`, `/sushi`, `/drinks`, and `/happy-hour` shows an "Order" button. Tapping it doesn't touch ChowNow or take payment (yet — this is designed to plug into a real order-entry system, or become one, later). It moves through three stages: **pending** (just tapped) → **entered** (staff checked with the table and actually entered it into the order system — it drops off the "Needs Entry" queue at this point) → **delivered** (confirmed by staff on `/table-orders-admin.html` *or* by the guest themselves, whose "Order" button becomes "Mark Received"). Full order-to-delivery timestamps are kept for 90 days, which is what powers the prep-time estimates and analytics KPIs below.
...
- Staff rewards (`/staff-rewards-admin.html`, admin only) — a punch card (same 10-punch mechanic as the customer loyalty card) earned by keeping the site itself up to date. Editing a single menu item (via `/edit-item.html` or the inline "Edit" link — not the bulk `/edit.html` save, since that endpoint is also used by one-off migration scripts and shouldn't rack up punches for those) auto-awards a punch for adding a photo, setting a price, and/or marking it a special, all independently, capped at 5 auto-awarded punches per staff member per day so repeatedly toggling the same field can't farm punches. Adding a new event on `/events-admin.html` auto-awards one too. Anything that can't be verified automatically (a social media post, going above and beyond) is a manual grant from an admin, exempt from the daily cap. Every staff member sees their own progress on `/account.html`; an admin redeems a completed card (whatever the reward actually is — a free shift meal, schedule preference, etc. — is a real-world decision to communicate separately) with an optional note for what was given.
