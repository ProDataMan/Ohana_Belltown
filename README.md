...
- **Shop checkout is built but not live yet.** Needs `SQUARE_ACCESS_TOKEN` + `SQUARE_LOCATION_ID` + `SQUARE_WEBHOOK_SIGNATURE_KEY` (see env table above) — not configured yet, even though the restaurant already has a Square account. Until then, `/shop` shows the 3 swag products with real prices, but the checkout button stays hidden (checked via `GET /api/swag/checkout-status`), same hidden-until-configured pattern. Products themselves (name/price/photos) can be managed on `/swag-admin.html` regardless.
