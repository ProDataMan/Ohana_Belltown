...
- **A real visual punch card, not just a number** (2026-09-14) — the signed-in "My Punch Card" state on `/rewards` (see above) previously rendered as a plain "N / 10 punches" pill. It's now `images/punch-card-bg.png` (a generated card — purple brand gradient, gold frame, the Ohana wordmark, "10 PUNCHES = ONE FREE ROLL" — with its lower half deliberately left visually clear) plus a live 10-dot overlay (`.punch-card-dots` in `style.css`, filled in by `renderPunchCard()` in `rewards.js`) — however many punches the guest actually has render as filled gold circles with a checkmark, the rest stay outlined. The dot count is capped at 10 even if the server total is briefly higher (a reward earned but not yet redeemed at the register), so the grid never overflows; "Free roll ready" still shows as a pill underneath once all 10 are filled. Punch count and card art are two separate layers on purpose — the count is different for every guest and changes in real time, so it's never baked into the image itself.
